...
- Present a prominent webpage, GitHub repository, product site, or recommended tool as a light macOS/Safari-style `link-card` instead of a dark terminal block or a naked long URL. Precompose the browser chrome, real address, title, and description into one static image so WeChat cannot reflow the design. For arbitrary external destinations that WeChat may strip from body HTML, attach a non-clickable CTA footer such as “点击文末『阅读原文』打开项目主页 →” and set the article's official source URL to the real destination. Do not imply the body card is clickable when the account cannot publish external links.
...
- Size rasterized card typography for its final article display width, not for the source PNG. At roughly 335px rendered width, target at least 17px for the project title, 12px for explanatory copy, and 10px for the address. Inspect the raster card at final scale before packaging.
